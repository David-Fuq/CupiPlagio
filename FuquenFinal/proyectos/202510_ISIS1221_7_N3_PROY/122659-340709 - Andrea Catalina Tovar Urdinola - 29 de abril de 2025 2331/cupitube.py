...
def cargar_cupitube(archivo: str) -> dict:
    ...
    while linea != "":
        ...
        d[encabezado[0]] = datos[0]
        d[encabezado[1]] = datos[1]
        d[encabezado[2]] = datos[2]
        d[encabezado[3]] = datos[3]
        d[encabezado[4]] = datos[4]
        d[encabezado[5]] = datos[5]
        d[encabezado[6]] = datos[6]
        d[encabezado[8]] = datos[8]
        d[encabezado[9][:-1]] = datos[9][:-1]
        
        
        if datos[7] not in dicc:
            lista = []
            lista.append(d)
            dicc[datos[7]] = lista
        
        elif datos[7] in dicc:
            dicc[datos[7]].append(d) 
    ...
    return dicc
# print(cargar_cupitube("cupitube.csv"))
...
def buscar_por_categoria_y_rango_suscriptores(cupitube: dict, suscriptores_min: int, suscriptores_max: int, categoria_buscada: str) -> list:
    ...
    for pais in cupitube:
        lista_cupitube = cupitube[pais]
        i = 0
        while i < len(lista_cupitube):
            datos = lista_cupitube[i]
            
            if suscriptores_min <= int(datos["subscribers"]) <= suscriptores_max and datos["category"] == categoria_buscada:
                lista.append(datos)
...
def buscar_cupitubers_por_pais_categoria_monetizacion(cupitube: dict, pais_buscado: str, categoria_buscada: str, monetizacion_buscada: str) -> list:
    ...
    lista = []
    pais = cupitube[pais_buscado]    
    i = 0
    
    while i < len(pais):
        datos = pais[i]
        
        if datos["category"] == categoria_buscada and datos["monetization_type"] == monetizacion_buscada:
            lista.append(datos)
...
def buscar_cupituber_mas_antiguo(cupitube: dict) -> dict:
    ...
    for pais in cupitube:
        ...
        i = 0
        while i < len(lista_pais):
            datos = lista_pais[i]
            inicio = datos["started"]
...
def obtener_visitas_por_categoria(cupitube: dict, categoria_buscada: str) -> int:
    ...
    for pais in cupitube:
        ...
        while i < len(lista_pais):
            datos = lista_pais[i]
            if datos["category"] == categoria_buscada:
                vistas += int(datos["video_views"])
...
def obtener_categoria_con_mas_visitas(cupitube: dict) -> dict:
    ...
    c = {}
    categoria = {"categoria":"","visitas":0}
        
    for pais in cupitube:
        lista_del_pais = cupitube[pais]
        i = 0
        
        while i < len(lista_del_pais):
            datos = lista_del_pais[i]
            
            if datos["category"] not in c:
                c[datos["category"]] = int(datos["video_views"])
    ...
    for category in c:
        ...
        if category == datos["category"] and int(vv) > int(categoria["visitas"]):
            categoria["categoria"] = category
            categoria["visitas"] = vv
...
def crear_correo_para_cupitubers(cupitube: dict) -> None:
    ...
    for pais in cupitube:
        lista_cada_pais = cupitube[pais]
        
        i = 0
        
        while i < len(lista_cada_pais):
            diccionario_cada_cupituber = lista_cada_pais[i]
            
            anio_mes_dia = diccionario_cada_cupituber["started"].split("-")
            ultimos_digitos_anio = int(anio_mes_dia[0]) - 2000
            ultimos_digitos_anio = str(ultimos_digitos_anio)
            
            mes = anio_mes_dia[1]
            mes = str(mes)
            
            if len(ultimos_digitos_anio) == 1 or len(mes):
                ultimos_digitos_anio = "0" + ultimos_digitos_anio
                mes 
            
            for cada_caracter in lista_cada_pais[i]["cupituber"]:
                
                if cada_caracter.isalnum():
                    nombre += cada_caracter
            
                correo = (nombre + "."+ultimos_digitos_anio+mes+"@cupitube.com")
