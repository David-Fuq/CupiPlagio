...
def cargar_cupitube(archivo: str) -> dict:
    ...
    diccionario = {}
    int_keys = ["rank", "subscribers", "videp_views", "video_count"]
    ...
    while line != "":
        ...
        for i in range(len(keys)):
            if keys[i] in int_keys:
                cupituber[keys[i]] = int(values[i])
            else:
                cupituber[keys[i]] = values[i]
        if cupituber["country"] not in diccionario:
            diccionario[cupituber["country"]] = []
        diccionario[cupituber["country"]].append(cupituber)
        line = data.readline().strip()
    return diccionario
...
def buscar_cupitubers_por_pais_categoria_monetizacion(cupitube: dict, pais_buscado: str, categoria_buscada: str, monetizacion_buscada: str) -> list:
    ...
    lista = []
    pais = cupitube.get(pais_buscado)
    for cupituber in pais:
    ...
    for pais in cupitube:
        for cupituber in cupitube[pais]:
            if cupituber["category"] == categoria_buscada:
                total_visitas += int(cupituber["video_views"])
...
def obtener_categoria_con_mas_visitas(cupitube: dict) -> dict:
    ...
    for pais in cupitube:
        for cupituber in cupitube[pais]:
            cat = cupituber["category"]
            visitas = int(cupituber["video_views"])
            if cat in visitas_categoria:
                visitas_categoria[cat] += visitas
            else:
                visitas_categoria[cat] = visitas
    ...
    for cat in visitas_categoria:
     visitas = visitas_categoria[cat]
...
def crear_correo_para_cupitubers(cupitube: dict) -> None:
    """
    Crea una dirección de correo electrónico para cada CupiTuber siguiendo un formato específico y la añade al 
    diccionario.
    Esta función modifica de forma permanente el diccionario recibido como parámetro, añadiendo una nueva llave
    ...
    for pais in cupitube:
        for cupituber in cupitube[pais]:
            ...
            x=""
            for car in nombre:
                if car in m or car in M or car in n :
                    if len(x) < 15:
                        x += car.lower()
            x += "."
            started= cupituber["started"]
            y= started[2: 4] + started[5: 7]
            x += y
            correo= x + "@cupitube.com"
            cupituber["correo"]= correo 
crear_correo_para_cupitubers(datos)
...
def recomendar_cupituber(cupitube: dict, suscriptores_min: int, suscriptores_max: int, fecha_minima: str, fecha_maxima: str, videos_minimos: int, palabra_clave: str) -> dict:
    ...
    for pais in cupitube:
        for cupituber in cupitube[pais]:
            if cupituber["category"] == categoria_objetivo and suscriptores_min <= int(cupituber["subscribers"]) <= suscriptores_max and fecha_minima <= cupituber["started"] <= fecha_maxima and int(cupituber["video_count"]) >= videos_minimos and palabra_clave in cupituber["description"].lower() :
               return cupituber
    return {}   
