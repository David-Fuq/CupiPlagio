# -*- coding: utf-8 -*-
"""
Created on Mon Apr 28 21:32:24 2025
...
def ejecutar_buscar_cupituber_mas_antiguo(cupitube: dict) -> None:
    ...
    for cupituber in cupitube["cupitubers"]:
       fecha_inicio = cupituber["started"]
       if fecha_mas_antigua is None or fecha_inicio < fecha_mas_antigua:
           fecha_mas_antigua = fecha_inicio
           cupituber_mas_antiguo = cupituber

       if cupituber_mas_antiguo is not None:
           print(f"El CupiTuber más antiguo es {cupituber_mas_antiguo['name']} y empezó en {cupituber_mas_antiguo['started']}.")
...
def ejecutar_obtener_visitas_por_categoria(cupitube: dict) -> None:
    ...
    categoria_usuario = input("Ingrese la categoría de CupiTubers: ")
    total_visitas = 0

    for cupituber in cupitube["cupitubers"]:
        if cupituber["category"] == categoria_usuario:
            total_visitas += cupituber["video_views"]

    print("El total de visitas para la categoría " + categoria_usuario + " es: " + str(total_visitas) + ".")
...
def ejecutar_obtener_categoria_con_mas_visitas(cupitube: dict) -> None:
    ...
    for cupituber in cupitube["cupitubers"]:
       ...
       visitas_max = visitas_por_categoria[categoria_max]

       print(f"La categoría con más visitas es {categoria_max} con {visitas_max} visitas.")
...
def ejecutar_recomendar_cupituber(cupitube: dict) -> None:
    ...
    minimo_subs = int(input("Ingrese el número mínimo de suscriptores: "))
    maximo_subs = int(input("Ingrese el número máximo de suscriptores: "))
    fecha_inicio = input("Ingrese la fecha de inicio (YYYY-MM-DD): ")
    fecha_fin = input("Ingrese la fecha de fin (YYYY-MM-DD): ")
    min_videos = int(input("Ingrese la cantidad mínima de videos publicados: "))
    palabra_clave = input("Ingrese una palabra clave que debe estar en la descripción: ").strip()

    if palabra_clave == "":
        print("La palabra clave no puede ser vacía.")
        return

    visitas_por_categoria = {}
    for c in cupitube["cupitubers"]:
        cat = c["category"]
        visitas_por_categoria[cat] = visitas_por_categoria.get(cat, 0) + c["video_views"]

    if not visitas_por_categoria:
        print("No hay categorías disponibles.")
        return

    categoria_mas_vistas = max(visitas_por_categoria, key=visitas_por_categoria.get)

    for c in cupitube["cupitubers"]:
        if (c["category"] == categoria_mas_vistas and
            minimo_subs <= c["subscribers"] <= maximo_subs and
            fecha_inicio <= c["started"] <= fecha_fin and
            c["video_count"] >= min_videos and
            palabra_clave.lower() in c["description"].lower()):
            
            print(f"El CupiTuber recomendado es {c['cupituber']}, que pertenece a la categoría con más visitas: {categoria_mas_vistas}.")
            mostrar_cupituber(c)
            return

    print("No se encontró ningún CupiTuber que cumpla con todos los criterios.")
