...
def ejecutar_buscar_cupitubers_por_pais_categoria_monetizacion(cupitube: dict) -> None:
    ...
    x = input("Ingrese su pais que desea buscar: ")
    y = input("Ingrese su categoria buscada: ")
    z = input("Ingrese su monetizacion buscada: ")
    
    
    resultado = ct.buscar_cupitubers_por_pais_categoria_monetizacion(cupitube, x, y, z)
    if resultado == []:
        print("No se encontraron CupiTubers que cumplan con los criterios.")
    else: 
        print(f"Los CupiTubers de {x} que pertenecen a la categoría {y} y tienen monetización {z} son:")
...
def ejecutar_buscar_cupituber_mas_antiguo(cupitube: dict) -> None:
    resultado = ct.buscar_cupituber_mas_antiguo(cupitube)
    
    if resultado == {}:
        print("No se encontró ningún CupiTuber en la base de datos.")
    else:
        print(f"El CupiTuber más antiguo es {resultado['cupituber']} y empezó en {resultado['started']}.")
...
def ejecutar_obtener_categoria_con_mas_visitas(cupitube: dict) -> None:
    ...
    categoria_info = ct.obtener_categoria_con_mas_visitas(cupitube)
    
    nombre_categoria = categoria_info["categoria"]
    visitas = categoria_info["visitas"]
    
    print(f"La categoría con más visitas es {nombre_categoria} con {visitas} visitas.")
...
def ejecutar_recomendar_cupituber(cupitube: dict) -> None:
    ...
    fecha_minima = input("Ingrese el límite inferior del rango de fechas (YYYY-MM-DD): ")
    fecha_maxima = input("Ingrese el límite superior del rango de fechas (YYYY-MM-DD): ")
    videos_minimos = int(input("Ingrese la cantidad mínima de videos: "))
    palabra_clave = input("Ingrese la palabra clave: ").strip()
