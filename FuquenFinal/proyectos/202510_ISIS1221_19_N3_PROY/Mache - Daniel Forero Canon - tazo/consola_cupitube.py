...
def ejecutar_buscar_cupitubers_por_pais_categoria_monetizacion(cupitube: dict) -> None:
    ...
    hay_cupitubers = len(cupitubers) > 0
    if hay_cupitubers:
...
def ejecutar_obtener_visitas_por_categoria(cupitube: dict) -> None:
    ...
    print(f"El total de visitas para la categoría {categoria} es: {visitas}.")
...
def ejecutar_obtener_categoria_con_mas_visitas(cupitube: dict) -> None:
    ...
    print(f"La categoría con más visitas es {resultado['categoria']} con {resultado['visitas']} visitas.")
...
def ejecutar_recomendar_cupituber(cupitube: dict) -> None:
    ...
    palabra_clave = input("Ingrese la palabra clave para la descripción: ")
    
    es_palabra_vacia = palabra_clave == ""
    if es_palabra_vacia:
        print("No se encontró un CupiTuber que cumpla con los criterios.")
        return
    
    cupituber = ct.recomendar_cupituber(cupitube, min_suscriptores, max_suscriptores, fecha_min, fecha_max, min_videos, palabra_clave)
    
    es_cupituber_encontrado = cupituber != {}
    if es_cupituber_encontrado:
        print("\nEl Cupituber recomendado tiene la siguiente información:")
        mostrar_cupituber(cupituber)
    else:
        print("No se encontró un CupiTuber que cumpla con los criterios.")
