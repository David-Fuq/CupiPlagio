...
def mostrar_cupituber(cupituber: dict) -> None:
    ...
    """
    Muestra la información de un CupiTuber.ç
    ...
    print((
        ...
        cupituber["name"], cupituber["rank"], cupituber["subscribers"],
...
def ejecutar_buscar_cupitubers_por_pais_categoria_monetizacion(cupitube: dict) -> None:
    ...
    tipo_monetizacion = input ("Tipo de Monetizacion: ")
    
    while pais [0].islower():
        print ("La primera letra de país debe ser mayuscula")
        pais = input ("Pais: ")
    
    
    resultado = ct.buscar_cupitubers_por_pais_categoria_monetizacion(cupitube,pais,categoria,tipo_monetizacion)
   
    if resultado == []:
        print ("No se encontraron CupiTubers que cumplan los criterios")
    
        
    else:
        print ("Los CupiTubers de {0} que pertenecen a la categoría {1} y tienen monetizacion {2} son: ".format(pais,categoria,tipo_monetizacion))
        print (mostrar_cupitubers(resultado))
...
def ejecutar_buscar_cupituber_mas_antiguo(cupitube: dict) -> None:
    ...
    nombre = antiguo ["name"]
    fecha = antiguo ["started"]
    print ("El CupiTuber más antiguo es {0} y empezó en {1}.".format(nombre,fecha))
...
def ejecutar_obtener_visitas_por_categoria(cupitube: dict) -> None:
    ...
    categoria_buscada = input ("Ingrese categoría: ")
    total = ct.obtener_visitas_por_categoria(cupitube, categoria_buscada)
    print ("El total de visitas para la categoria {0} es: {1}.".format (categoria_buscada,total))
...
def ejecutar_obtener_categoria_con_mas_visitas(cupitube: dict) -> None:
    ...
    numero_visitas = ct.obtener_visitas_por_categoria(cupitube, categoria)
    print ("La categoría con más visitas es {0} con {1} visitas.".format (categoria,numero_visitas))
...
def ejecutar_crear_correo_para_cupitubers(cupitube: dict) -> None:
    ...
    if "USA" in cupitube and cupitube["USA"] != [] and cupitube["USA"][0] is not None:
        ...
        print("El correo para el primer Cupituber de 'USA' con nombre '" + cupituber["name"] + "' es: " + cupituber["correo"] + ".")
        

def ejecutar_recomendar_cupituber(cupitube: dict) -> None:
    
   
    suscriptores_min = int (input ("Ingrese minimo de suscriptores: "))
    suscriptores_max = int (input ("Ingrese maximo de suscriptores: "))
    fecha_minima = input ("Ingrese fecha minima en formato AAAA-MM-DD: ")
    while fecha_minima [4] != "-" or fecha_minima [7]!= "-":
        print ("Formato de fecha invalido")
        fecha_minima = input ("Ingrese fecha en formato AAAA-MM-DD: ")
    fecha_maxima = input("Ingrese fecha maxima en formato AAAA-MM-DD: ")
    while fecha_maxima [4] != "-" or fecha_maxima [7] != "-":
        fecha_maxima = input ("Ingrese fecha maxima en formato AAAA-MM-DD: ")
    videos_minimos = int (input ("Ingrese cantidad minima de videos: "))
    palabra_clave = input ("Ingrese palabra clave de la descripcion: ")
    while palabra_clave == "" or palabra_clave == " ":
        print ("Debe ingresar una palabra clave")
        palabra_clave = input ("Ingrese una palabra clave: ")
    
    
    recomendado = ct.recomendar_cupituber(cupitube, suscriptores_min, suscriptores_max, fecha_minima, fecha_maxima, videos_minimos, palabra_clave)
    ...
    """ 
    Ejecuta la función de  que recomienda al
    vlogsprimer CupiTuber que cumpla con todos los criterios de búsqueda especificados:
