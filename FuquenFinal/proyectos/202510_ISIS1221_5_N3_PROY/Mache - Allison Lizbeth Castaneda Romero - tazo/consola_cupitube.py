...
def ejecutar_buscar_cupitubers_por_pais_categoria_monetizacion(cupitube: dict) -> None:
    ...
    monetización = input("Ingrese el tipo de monetizacion: ")
    ...
    for i in cupitube:
        ...
        if c["country"] == pais:
            if c["category"] == categoria:
                if c["monetization_type"] == monetización:
                    cupitubers.append(c)
                    if len(cupitubers) ==0:
                        print("No se encontraron CupiTubers que cumplan con los criterios.")
                    else:
                        print("Los CupiTubers de",pais," que pertenecen a la categoría",categoria, "y tienen monetización" ,monetización," son:")
...
def ejecutar_buscar_cupituber_mas_antiguo(cupitube: dict) -> None:
    ...
    """    
    cupitube_mas_antiguo=""
    fecha_mas_antiguo="YYYY-MM-DD"
    for i in cupitube:
        c= cupitube[i]
        fecha_inicio= c["star_date"]
        if fecha_inicio< fecha_mas_antiguo:
            fecha_mas_antiguo=fecha_inicio
            cupitube_mas_antiguo =c["name"]
        print("El CupiTuber más antiguo es",cupitube_mas_antiguo," y empezó en ",fecha_mas_antiguo,".")
...
def ejecutar_obtener_visitas_por_categoria(cupitube: dict) -> None:
    ...
    for i in cupitube:
        c= cupitube[i]
        if c["category"] == categoria:
            total_de_visitas += c["video_views"]
        print("El total de visitas para la categoría",categoria," es:",total_de_visitas,".")
...
def ejecutar_obtener_categoria_con_mas_visitas(cupitube: dict) -> None:
    ...
    respuesta =obtener_categoria_con_mas_visitas(cupitube)
    categoria =respuesta["category"]
    visitas= respuesta["video_views"]
    print("La categoría con más visitas es" ,categoria," con" , visitas,"visitas.")
